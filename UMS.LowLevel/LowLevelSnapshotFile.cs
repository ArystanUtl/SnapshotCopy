...
public class LowLevelSnapshotFile : IDisposable
{
    ...
    public unsafe LowLevelSnapshotFile(string path)
    {
        ...
        {
            File.AppendAllText("log1.txt",$"Warning: Decreasing entry offset count from {entryOffsetCount} to {EntryType.Count} to match entry type count.\n");
        ...
        File.AppendAllText("log1.txt","Loading managed heap sections...\n");
        ...
        File.AppendAllText("log1.txt","Sorting managed heap sections...\n");
    ...
    public Span<T> ReadSingleValueTypeArrayChapterElement<T>(EntryType entryType, int offset) where T : unmanaged
    {
        ...
        // File.AppendAllText("log1.txt",$"Warning: Falling back to byte array for single value type array chapter element of type {entryType} offset {offset}\n");
    ...
    public Span<T> ReadValueTypeChapter<T>(EntryType entryType, int startOffset, int count) where T : unmanaged
    {
        ...
        //Fall back to byte array
        File.AppendAllText("log1.txt",$"Warning: Falling back to byte array for value type chapter of type {entryType}\n");
        var rawData = ReadChapterBody(entryType, startOffset, count);
        return MemoryMarshal.Cast<byte, T>(rawData);
    }
