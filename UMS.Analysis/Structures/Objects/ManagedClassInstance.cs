...
public readonly struct ManagedClassInstance
{
    ...
    public ManagedClassInstance(SnapshotFile file, RawManagedObjectInfo info, ManagedClassInstance? parent = null, int depth = 0, LoadedReason loadedReason = LoadedReason.GcRoot, int fieldIndexOrArrayOffset = int.MinValue)
    {
        ...
        if (IsArray)
        {
            ...
            {
                File.AppendAllText("log1.txt","WARNING: Skipping uninitialized array type\n");
                Fields = Array.Empty<IFieldValue>();
                return;
            }
            ...
            if (oldArrayElementCount != arrayElementCount)
                File.AppendAllText("log1.txt",$"WARNING: Array length mismatch for {file.GetTypeName(info.TypeDescriptionIndex)} at 0x{info.SelfAddress:X} (expected {oldArrayElementCount}, got {arrayElementCount})\n");
    ...
    private IFieldValue[] ReadFields(SnapshotFile file, Span<byte> data, int depth)
    {
        ...
        if (depth > 370)
        {
            File.AppendAllText("log1.txt",$"Stopped reading fields due to too-deeply nested object at depth {depth} (this object is of type {file.GetTypeName(TypeInfo.TypeIndex)}, parent is of type {file.GetTypeName(TypedParent.TypeInfo.TypeIndex)})\n");
            return Array.Empty<IFieldValue>();
        }
