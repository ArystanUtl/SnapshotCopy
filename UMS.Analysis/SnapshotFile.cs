...
public class SnapshotFile : LowLevelSnapshotFile
{
    ...
    public void LoadManagedObjectsFromGcRoots()
    {
        ...
        File.AppendAllText("log1.txt",$"Processing {gcRoots.Length} GC roots...\n");
        foreach (var gcHandle in gcRoots) 
            GetOrCreateManagedClassInstance(gcHandle);
        
        File.AppendAllText("log1.txt",$"Found {_managedClassInstanceCache.Count} managed objects in {(DateTime.Now - start).TotalMilliseconds}ms\n");
    }
    ...
    public void LoadManagedObjectsFromStaticFields()
    {
        ...
        File.AppendAllText("log1.txt",$"Processing static field info for {allStaticFields.Length} types...\n");
        ...
        File.AppendAllText("log1.txt",$"Found {_managedClassInstanceCache.Count - initialCount} additional managed objects from static fields in {(DateTime.Now - start).TotalMilliseconds}ms\n");
    }
    ...
    public RawManagedObjectInfo ParseManagedObjectInfo(ulong address)
    {
        ...
        if (!TypeIndicesByPointer.TryGetValue(info.TypeInfoAddress, out info.TypeDescriptionIndex))
        {
            ...
            {
                File.AppendAllText("log1.txt",$"WARNING: Failed to resolve type for object at {address:X} - type info address {info.TypeInfoAddress:X} resulted in type index {info.TypeDescriptionIndex}\n");
    ...
    private int GetObjectSizeFromStringInBytes(RawManagedObjectInfo info, Span<byte> heap)
    {
        ...
        {
            File.AppendAllText("log1.txt",$"Warning - string length is invalid: {length}\n");
            return 0;
        }
    ...
    private int GetObjectSizeFromArrayInBytes(RawManagedObjectInfo info, Span<byte> heap)
    {
        ...
        {
            File.AppendAllText("log1.txt",$"Warning: Reducing array length {arrayLength} to {heap.Length} because the heap doesn't contain all the data.\n");
